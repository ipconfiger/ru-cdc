...
impl Workers {
    ...
    pub fn start(&mut self, size: usize){
        ...
        for thread_id in 0..size {
            ...
            thread::spawn(move || {
                worker_body(thread_id, &mut the_pool, &mut the_mapping);
            });
        }
    }
...
fn worker_body(thread_id: usize, pool: &mut Pool, mapping: &mut TableMetaMapping) {
